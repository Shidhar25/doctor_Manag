...
const Appointment = () => {
    ...
    const handleNotification = async () => {
        if (!token) {
            toast.warning('Please login to get notifications')
            return navigate('/login')
        }
        try {
            const { data } = await axios.post(backendUrl + '/api/user/notify-slot', {
                docId,
            }, { headers: { token } })

            if (data.success) {
                toast.success("You'll be notified when a slot becomes available")
            } else {
                toast.error(data.message)
            }
        } catch (error) {
                ...
                <div className='flex items-center gap-4 my-6'>
                    {!consultationResult ? (
                        <button 
                            ...
                            className='bg-primary text-white text-sm font-light px-12 py-3 rounded-full flex items-center gap-2'
                        >
                    ...
                    <button 
                        onClick={() => window.open(`tel:${docInfo.phone}`)}
                        className='bg-green-500 text-white text-sm font-light px-8 py-3 rounded-full flex items-center gap-2 hover:bg-green-600 transition-colors'
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <button 
                        ...
                        className='bg-purple-500 text-white text-sm font-light px-8 py-3 rounded-full flex items-center gap-2 hover:bg-purple-600 transition-colors'
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
